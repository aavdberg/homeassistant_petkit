...
class PetkitNotificationManager:
    ...
    def _notif_enabled(self, device: Any, setting_attr: str | None) -> bool:
        ...
        Missing attributes (device doesn't expose this setting) are treated as
        enabled so notifications work on all devices by default.
        """
        if setting_attr is None:
            return True
        setting_value = _safe_get(device, "settings", setting_attr, default=None)
